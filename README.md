...
- **内置对话界面**：管理后台自带聊天窗口，支持流式输出、多轮对话与原始响应调试。
...
└─────────┼──────────────────────────────┘
     ...
     ┌─────┴──────┬─────────┐
     ▼            ▼         ▼
  上游 vLLM    上游 Ollama  上游 云API
  (端口8000)   (端口11434)  (DeepSeek等)
```
...
| GET | `/admin/keys/{id}/reveal` | 获取密钥明文（管理员） |
...
| 用量统计 | 所有用户 | 密钥使用次数统计 |
| 密钥管理 | 所有用户 | 密钥列表、创建、吊销、绑定上游 |
| 上游管理 | 管理员 | 多上游服务配置（名称、地址、API Key、默认状态） |
| 用户管理 | 管理员 | 后台用户管理（管理员/查看者角色） |
| 对话 | 所有用户 | 内置聊天窗口，支持流式输出与原始响应调试 |
...
2. 在**密钥管理**中创建或编辑密钥，选择绑定的上游
3. 调用方使用该密钥请求 `/v1/chat/completions`，网关自动转发到对应上游
