...
- 🛡️ 管理接口 `MASTER_KEY` 保护，支持多用户（管理员/普通用户）；普通用户仅见自己名下密钥用量，并可用其进行对话测试
- 🗄️ SQLite 持久化，零外部依赖
- 📊 密钥用量统计（最后使用时间、请求次数）
...
| 页面 | 权限 | 说明 |
|------|:----:|------|
| 系统概览 | 管理员 | 网关状态、密钥数量、上游数量等概览信息 |
| 用量统计 | 全部用户 | 用量趋势 / 密钥排行 / 明细矩阵；**普通用户只统计自己名下密钥的用量** |
| 密钥管理 | 管理员 | 密钥列表（含归属列）、创建（可指定归属用户、日期+时间选择过期）、重置、吊销、转移归属 |
| 上游管理 | 管理员 | 多上游服务配置（名称、地址、API Key、协议类型、可用模型、默认状态） |
| 用户管理 | 管理员 | 后台用户管理（管理员/普通用户角色） |
| 对话测试 | 全部用户 | 内置聊天窗口：流式输出、Markdown 渲染、消息工具条、原始响应调试；**普通用户只能选择/回显/使用自己名下密钥** |

### 密钥管理

密钥管理页为**管理员**专属：

- **归属用户**：每个密钥可选归属某**普通用户**账号——归属后该普通用户登录只能看到 / 统计 / 测试**自己名下**密钥；**不归属 = 系统密钥，仅管理员可见**。归属可随时在列表「归属」列转移或解除（普通用户被删除时，其名下密钥自动回到系统密钥，避免悬空）。
- **创建密钥**：填写名称 → 绑定上游（可留空走默认上游）→（可选）选择归属用户 → 用「日期 + 时间」选择器设置过期时间（留空永不过期）→ 创建后弹窗回显明文 `sk-`，请立即复制保存。
...
- **从列表选择**：下拉列出你**名下**的密钥（管理员为全部密钥），选择后自动获取完整 Key 用于测试
- **手动输入**：粘贴任何 `sk-` 密钥进行测试

> 说明：普通用户只能选择 / 回显 / 使用**自己名下**的密钥。网关 `/v1/*` 只认 `sk-` 凭据本身，因此「手动输入」对任何知晓明文密钥的调用方都有效——这是 bearer 凭证模型的固有边界。
...
- **哈希鉴权 + 明文回显**：`/v1/*` 网关鉴权只比对 `sha256` 哈希；明文 `sk-` 同时落库，仅在内网管理后台回显（创建/重置弹窗、对话测试「从列表选择」）——管理员可回显任意密钥，**普通用户仅能回显自己名下密钥**；列表与查询接口一律只返回 `sk-ab12****` 前缀。
- **最小授权**：普通用户登录只显示「用量统计 / 对话测试」，且用量统计按**归属该普通用户的密钥**过滤；系统概览、密钥/上游/用户管理与全站统计仅管理员可访问。
